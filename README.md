...
•   ModuleNotFoundError: Если возникает ошибка, связанная с отсутствием модулей, убедитесь, что вы выполнили команду pip install -r requirements.txt.

•   ValueError: Bot token not found in config.json: Проверьте, что файл config.json находится в корневой директории проекта, и что вы корректно указали токен вашего бота в этом файле.

•   **Бот не отвечает:**
    *   Проверьте, что бот запущен (выполняется скрипт main.py).
    *   Проверьте, что ваше интернет-соединение стабильно.
    *   Убедитесь, что у бота нет проблем с доступом к Telegram API.
